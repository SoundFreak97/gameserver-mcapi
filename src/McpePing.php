...
namespace Gameserver\MinecraftAPI;
    ...
    public static function ping($host, $port, $debug = false)
    {
        ...
        if (!$stream) {
            ...
            if ($debug) {
                return array(
                    "online" => false,
                    "error" => "Unable to connect to " . $host . ":" . $port . ": " . $errstr,
                    "hostname" => $host,
                    "port" => $port,
                    "errno" => $errno,
                    "errstr" => $errstr
        ...
        if (count($buf) < 1 || $buf[0] != "\x1c") {
            ...
            if ($debug) {
                return array(
                    "online" => false,
                    "error" => "Unable to get a response / valid response",
                    "hostname" => $host,
                    "port" => $port,
                    "errno" => $errno,
                    "errstr" => $errstr,
        ...
        return array(
            "online" => true,
            "motd" => $ping_parts[1],
            "protocol" => (int) $ping_parts[2],
            "version" => $ping_parts[3],
            "players" => (int) $ping_parts[4],
            "max_players" => (int) $ping_parts[5],
            "hostname" => $host,
            "port" => $port
        );
    }
